...
public partial class myOrder_Step2 : SecurityCheckDealer
{
    ...
    protected void lvDataList_ItemDataBound(object sender, ListViewItemEventArgs e)
    {
        if (e.Item.ItemType == ListViewItemType.DataItem)
        {
            ...
            {
                lt_Msg.Text = "<div class=\"text-danger\">{0}<b>{1}</b> &rarr; {2}</div>".FormatThis(
                    this.GetLocalResourceObject("tip4").ToString(), Get_BuyQty, Get_MinQty);
    ...
    protected void lvDataList_ItemCommand(object sender, ListViewCommandEventArgs e)
    {
        if (e.Item.ItemType == ListViewItemType.DataItem)
        {
            ...
            {
                case "doEdit":
                    //判斷:輸入值Get_BuyQty >= 最小量Get_MinQty,未成立直接帶最小量
                    if (Get_BuyQty < Get_MinQty)
                    {
