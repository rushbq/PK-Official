...
public partial class myQA_List : System.Web.UI.Page
{
    ...
    private void LookupDataList(int pageIndex)
    {
        ...
        {
            search.Add((int)mySearch.ClassID, Req_DataID);

            PageParam.Add("cls=" + Server.UrlEncode(Req_DataID));
        }
        ...
        if (query != null)
        {
            var topData = query.FirstOrDefault();

            //填入header資料
            this.lt_ClassHeader.Text = topData.ClassName;
    ...
    public string Req_DataID
    {
        get
        {
            String DataID = Request.QueryString["cls"];

            return string.IsNullOrEmpty(DataID) ? "" : DataID.ToString();
        }
        set
        {
    ...
    public string PageUrl
    {
        get
        {
            return "{0}{1}/FAQ-List".FormatThis(webUrl, Req_Lang);
        }
        set
        {
            this._PageUrl = value;
