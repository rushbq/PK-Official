...
public partial class myToyProd_ProdList : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            ...
            {
                //** 次標題 **
                this.Page.Title = Resources.resPublic.title_科學玩具;
                ...
                lt_ToolCnt.Text = GetData_ToolCnt().ToString();
    ...
    private void LookupDataList(int pageIndex)
    {
        ...
        {
            //[參數宣告] - 設定本頁Url(末端無須加 "/")
            this.ViewState["Page_Url"] = "{0}Search/RobotKit/{1}".FormatThis(
                    Application["WebUrl"]
    ...
    private Int32 GetData_ToolCnt()
    {
        ...
        try
        {
            ...
            SELECT COUNT(*) FROM (
                SELECT GP.Model_No
                FROM Prod GP
                INNER JOIN [ProductCenter].dbo.Prod_Item myData WITH (NOLOCK) ON GP.Model_No = myData.Model_No
            WHERE (GP.Display = 'Y') 
            ...
            //排除Toy關聯
            sql += @" AND (myData.Model_No NOT IN (
                        SELECT Rel.Model_No
                        FROM [ProductCenter].dbo.ProdToy_Class_Rel_ModelNo Rel))";

            //[查詢條件] - 產品類別(Tool)
            if (!string.IsNullOrEmpty(Req_ClassID) && (!Req_ClassID.ToUpper().Equals("ALL")))
            {
                sql += " AND (myData.Class_ID = @Class_ID)";
                cmd.Parameters.AddWithValue("Class_ID", Req_ClassID);
            }
            sql += " ";
            ...
            if (!string.IsNullOrEmpty(Req_Keyword))
            {
                sql += @"
                    AND (
                      (UPPER(myData.Model_No) LIKE '%' + UPPER(@Keyword) + '%')
                      OR (myData.Model_Name_##langcode## LIKE '%' + @Keyword + '%')
            ...
            sql += " GROUP BY GP.Model_No) AS TbCnt";

            sql = sql.Replace("##langcode##", fn_Language.Param_Lang);
    ...
    protected void btn_Search_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            SBUrl.Append("{0}Search/RobotKit/{1}/".FormatThis(
                    Application["WebUrl"]
