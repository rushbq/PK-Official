...
public partial class myProd_ProdList : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            if (!IsPostBack)
            {
                ...
                lt_ToyCnt.Text = GetData_ToyCnt().ToString();
    ...
    private void LookupDataList(int pageIndex)
    {
        ...
        {
            //[參數宣告] - 設定本頁Url(末端無須加 "/")
            this.ViewState["Page_Url"] = "{0}Search/Tool/{1}".FormatThis(
                    Application["WebUrl"]
    ...
    private Int32 GetData_ToyCnt()
    {
        ...
        try
        {
            ...
            //Toy關聯
            sql += @" AND (myData.Model_No IN (
                        SELECT Rel.Model_No
                        FROM [ProductCenter].dbo.ProdToy_Class_Rel_ModelNo Rel";
            //[查詢條件] - 產品類別(TOY)
            if (!string.IsNullOrEmpty(Req_ClassID) && (!Req_ClassID.ToUpper().Equals("ALL")))
            {
                sql += " WHERE (Rel.Class_ID = @Class_ID)";
                cmd.Parameters.AddWithValue("Class_ID", Req_ClassID);
            }
            sql += " ))";
    ...
    protected void btn_Search_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            SBUrl.Append("{0}Search/Tool/{1}/".FormatThis(
                    Application["WebUrl"]
