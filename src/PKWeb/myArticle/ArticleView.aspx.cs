...
public partial class myNews_ArticleView : System.Web.UI.Page
{
    ...
    private void LookupData_News()
    {
        try
        {
            ...
            using (SqlCommand cmd = new SqlCommand())
            {
                ...
                //[SQL] - 資料查詢
                SBSql.AppendLine(" SELECT Base.Article_Title, Base.Article_Desc, Base.Article_PubDate, Base.Group_ID, Base.Article_ID, Base.Article_Pic ");
                SBSql.AppendLine("  , Sub.Block_Desc, Sub.Block_Pic ");
                SBSql.AppendLine(" FROM Article Base ");
                SBSql.AppendLine("  INNER JOIN Article_Block Sub ON Base.Article_ID = Sub.Article_ID ");
                ...
                using (DataTable DT = dbConn.LookupDT(cmd, out ErrMsg))
                {
                    ...
                    if (DT.Rows.Count > 0)
                    {
                        ...
                        lt_Title.Text = DT.Rows[0]["Article_Title"].ToString();

                        Literal lt_Date = (Literal)this.lvDataList.FindControl("lt_Date");
                        lt_Date.Text = DT.Rows[0]["Article_PubDate"].ToString().ToDateString("yyyy/MM/dd");

                        this.lt_BackUrl.Text = "<a href=\"{0}\">Back</a>".FormatThis(
                                Application["WebUrl"] + "Article/" + DT.Rows[0]["Article_PubDate"].ToString().ToDateString("yyyy")
                            );

                        //Meta資訊
                        meta_Title = "{0} | {1}".FormatThis(DT.Rows[0]["Article_Title"].ToString(), Application["WebName"].ToString());
                        meta_Desc = DT.Rows[0]["Article_Desc"].ToString().Left(100);//HttpUtility.HtmlDecode(DT.Rows[0]["Block_Desc"].ToString()).Left(100);
                        meta_Url = "{0}Article/View/{1}".FormatThis(
                            Application["WebUrl"].ToString()
                            , Cryptograph.MD5Encrypt(DT.Rows[0]["Article_ID"].ToString(), Application["DesKey"].ToString())
                            );
                        meta_Image = "{0}Article/{1}/{2}".FormatThis(
                            Application["File_WebUrl"].ToString() + Param_FileWebFolder
                            , DT.Rows[0]["Group_ID"].ToString()
                            , DT.Rows[0]["Article_Pic"].ToString()
                            );
    ...
    protected void lvDataList_ItemDataBound(object sender, ListViewItemEventArgs e)
    {
        if (e.Item.ItemType == ListViewItemType.DataItem)
        {
            ...
            if (!string.IsNullOrEmpty(GetPic))
            {
                ...
                //顯示Html
                lt_Pic.Text = "<img data-original=\"{0}\" src=\"{1}js/lazyload/grey.gif\" class=\"img-responsive lazy\" alt=\"Article\" />".FormatThis(
