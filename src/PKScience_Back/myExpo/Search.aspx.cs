...
public partial class Expo_Search : SecurityCheck
{
    ...
    private void LookupDataList(int pageIndex)
    {
        ...
        {
            //[參數宣告] - 設定本頁Url(末端無須加 "/")
            this.ViewState["Page_Url"] = Application["WebUrl"] + "Activity/Search";
        ...
        try
        {
            if (e.Item.ItemType == ListViewItemType.DataItem)
            {
                ...
                using (SqlCommand cmd = new SqlCommand())
                {
                    ...
                    {
                        //刪除整個Folder
                        string fileUrl = @"{0}Activity\{1}\".FormatThis(fn_Param.File_DiskUrl, Get_DataID);
    ...
    protected void btn_Search_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            SBUrl.Append("{0}Activity/Search/?srh=1".FormatThis(Application["WebUrl"]));
