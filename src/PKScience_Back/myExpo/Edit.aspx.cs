...
public partial class Expo_Edit : SecurityCheck
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            ...
            {
                //[權限判斷]
                if (fn_CheckAuth.CheckAuth("310", out ErrMsg) == false)
                {
                ...
                //** 設定程式編號(重要) **
                if (false == setProgIDs.setID(this.Master, "300", "310"))
                {
            ...
            using (SqlCommand cmd = new SqlCommand())
            {
                ...
                using (DataTable DT = dbConn.LookupDT(cmd, out ErrMsg))
                {
                    ...
                    else
                    {
                        ...
                        this.show_sDate_Act.Text = DT.Rows[0]["ActStartDate"].ToString().ToDateString("yyyy/MM/dd");
                        this.show_eDate_Act.Text = DT.Rows[0]["ActEndDate"].ToString().ToDateString("yyyy/MM/dd");
                        this.tb_StartDate_Act.Text = DT.Rows[0]["ActStartDate"].ToString().ToDateString("yyyy/MM/dd");
                        this.tb_EndDate_Act.Text = DT.Rows[0]["ActEndDate"].ToString().ToDateString("yyyy/MM/dd");
    ...
    private void LookupData_Detail()
    {
        try
        {
            using (SqlCommand cmd = new SqlCommand())
            {
                ...
                SBSql.AppendLine(" SELECT Lang.LangName, Lang.LangCode, Base.Expo_ID ");
                SBSql.AppendLine("     , (CASE WHEN Base.Expo_ID IS NULL THEN 'N' ELSE 'Y' END) IsSet ");
                SBSql.AppendLine(" FROM PKSYS.dbo.Param_Language Lang ");
                SBSql.AppendLine("     LEFT JOIN Expo Base ON Lang.LangCode = Base.LangCode AND Base.Group_ID = @DataID ");
                ...
                using (DataTable DT = dbConn.LookupDT(cmd, out ErrMsg))
                {
                    ...
                    for (int row = 0; row < DT.Rows.Count; row++)
                    {
                        ...
                        string ExpoID = DT.Rows[row]["Expo_ID"].ToString();
                        ...
                        html.AppendLine("<a href=\"{0}\" class=\"{1}\"><i class=\"{2}\"></i>&nbsp;{3}</a>".FormatThis(
                                "{0}Activity/Edit/Detail/{1}/{2}/{3}".FormatThis(
                                        Application["WebUrl"]
                    ...
                    //輸出Html
                    this.lt_ExpoSet.Text = html.ToString();
                }
            }
        }
        catch (Exception)
    ...
    private void Add_Data()
    {
        using (SqlCommand cmd = new SqlCommand())
        {
            ...
            SBSql.AppendLine("  SELECT ISNULL(MAX(Group_ID) ,0) + 1 FROM Expo_Group ");
            SBSql.AppendLine(" );");
            ...
            //[SQL] - 資料新增
            SBSql.AppendLine(" INSERT INTO Expo_Group( ");
            SBSql.AppendLine("  Group_ID, Group_Name, StartTime, EndTime, ActStartDate, ActEndDate, Display, Sort, onIndex");
            SBSql.AppendLine("  , Create_Who, Create_Time");
            SBSql.AppendLine(" ) VALUES ( ");
            SBSql.AppendLine("  @NewID, @Group_Name, @StartTime, @EndTime, @ActStartDate, @ActEndDate, @Display, @Sort, @onIndex");
            ...
            {
                //將其他資料皆設為N
                SBSql.AppendLine(" UPDATE Expo_Group SET onIndex = 'N' WHERE (Group_ID <> @NewID); ");
            }
            ...
            cmd.Parameters.AddWithValue("ActStartDate", Convert.ToDateTime(this.tb_StartDate_Act.Text.ToDateString("yyyy-MM-dd")));
            cmd.Parameters.AddWithValue("ActEndDate", Convert.ToDateTime(this.tb_EndDate_Act.Text.ToDateString("yyyy-MM-dd")));
            ...
            //更新本頁Url
            string thisUrl = "{0}Activity/Edit/{1}/".FormatThis(
                        Application["WebUrl"]
    ...
    private void Edit_Data()
    {
        using (SqlCommand cmd = new SqlCommand())
        {
            ...
            //[SQL] - 資料更新
            SBSql.AppendLine(" UPDATE Expo_Group ");
            ...
            {
                //將其他資料皆設為N
                SBSql.AppendLine(" UPDATE Expo_Group SET onIndex = 'N' WHERE (Group_ID <> @DataID); ");
            }
    ...
    protected void lbtn_Delete_Click(object sender, EventArgs e)
    {
        try
        {
            using (SqlCommand cmd = new SqlCommand())
            {
                ...
                //[SQL] - 資料更新
                SBSql.AppendLine(" DELETE FROM Expo WHERE (Group_ID = @DataID); ");
                SBSql.AppendLine(" DELETE FROM Expo_Group WHERE (Group_ID = @DataID); ");
                ...
                {
                    //刪除整個Folder
                    string fileUrl = @"{0}Activity\{1}\".FormatThis(fn_Param.File_DiskUrl, Param_thisID);
    ...
    public string Page_CurrentUrl
    {
        get
        {
            return "{0}Activity/Edit/{1}/".FormatThis(
                Application["WebUrl"]
    ...
    public string Page_SearchUrl
    {
        get
        {
            ...
            {
                Url = "{0}Activity/Search/".FormatThis(Application["WebUrl"]);
            }
            else
            {
