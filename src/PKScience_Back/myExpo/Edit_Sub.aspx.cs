...
public partial class Expo_Edit_Sub : SecurityCheck
{
    ...
    private void LookupData()
    {
        try
        {
            ...
            using (SqlCommand cmd = new SqlCommand())
            {
                ...
                //[SQL] - 資料查詢
                SBSql.AppendLine(" SELECT Base.Expo_ID, Base.Expo_Title, Base.Expo_SubTitle, Base.Expo_Desc, Base.Expo_ListPic, Base.Expo_BigPic, Base.Expo_Url");
                ...
                using (DataTable DT = dbConn.LookupDT(cmd, out ErrMsg))
                {
                    ...
                    else
                    {
                        ...
                        this.tb_Expo_SubTitle.Text = DT.Rows[0]["Expo_SubTitle"].ToString();
                        this.tb_Expo_Desc.Text = DT.Rows[0]["Expo_Desc"].ToString();
                        this.tb_Expo_Url.Text = DT.Rows[0]["Expo_Url"].ToString();

                        //檔案Url - 小圖
                        string myFile = DT.Rows[0]["Expo_ListPic"].ToString();
                        ...
                        //檔案Url - 大圖
                        string myFile_Big = DT.Rows[0]["Expo_BigPic"].ToString();
    ...
    private void Add_Data(List<TempParam> ITempList)
    {
        using (SqlCommand cmd = new SqlCommand())
        {
            ...
            SBSql.AppendLine("  , Expo_Title, Expo_SubTitle, Expo_Desc, Expo_ListPic, Expo_BigPic, Expo_Url");
            ...
            cmd.Parameters.AddWithValue("Expo_SubTitle", this.tb_Expo_SubTitle.Text.Trim());
            cmd.Parameters.AddWithValue("Expo_Desc", this.tb_Expo_Desc.Text);
            cmd.Parameters.AddWithValue("Expo_ListPic", pic1);
            cmd.Parameters.AddWithValue("Expo_BigPic", pic2);
            cmd.Parameters.AddWithValue("Expo_Url", this.tb_Expo_Url.Text);
            ...
            //更新本頁Url
            string thisUrl = "{0}Activity/Edit/Detail/{1}/{2}/{3}/".FormatThis(
                        Application["WebUrl"]
    ...
    private void Edit_Data(List<TempParam> ITempList)
    {
        using (SqlCommand cmd = new SqlCommand())
        {
            //宣告
            StringBuilder SBSql = new StringBuilder();
            string pic1 = this.hf_OldFile_Small.Value, pic2 = this.hf_OldFile_Big.Value;
            ...
            SBSql.AppendLine(" SET Expo_Title = @Expo_Title, Expo_SubTitle = @Expo_SubTitle ");
            SBSql.AppendLine("  , Expo_Desc = @Expo_Desc, Expo_ListPic = @Expo_ListPic, Expo_BigPic = @Expo_BigPic, Expo_Url = @Expo_Url");
    ...
    protected void lbtn_DelFile_Small_Click(object sender, EventArgs e)
    {
        try
        {
            using (SqlCommand cmd = new SqlCommand())
            {
                ...
                //[SQL] - 資料更新
                SBSql.AppendLine(" UPDATE Expo SET Expo_ListPic = NULL WHERE (Expo_ID = @DataID) ");
    ...
    protected void lbtn_DelFile_Big_Click(object sender, EventArgs e)
    {
        try
        {
            using (SqlCommand cmd = new SqlCommand())
            {
                ...
                //[SQL] - 資料更新
                SBSql.AppendLine(" UPDATE Expo SET Expo_BigPic = NULL WHERE (Expo_ID = @DataID) ");
    ...
    public string Page_CurrentUrl
    {
        get
        {
            return "{0}Activity/Edit/Detail/{1}/{2}/{3}/".FormatThis(
                Application["WebUrl"]
                , Cryptograph.MD5Encrypt(Param_parentID, fn_Param.DesKey)
                , Param_LangCode
    ...
    public string Param_FileFolder
    {
        get
        {
            return this._Param_FileFolder != null
                ...
                : @"{0}Activity\{1}\".FormatThis(fn_Param.File_DiskUrl, Param_parentID);
        }
        set
        {
    ...
    public string Param_WebFolder
    {
        get
        {
            return this._Param_WebFolder != null
                ...
                : @"{0}Activity/{1}/".FormatThis(fn_Param.File_WebUrl, Param_parentID);
        }
        set
        {
