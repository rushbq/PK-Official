...
public partial class Edit_DetailBox : SecurityCheck
{
    ...
    private void LookupData()
    {
        try
        {
            ...
            using (SqlCommand cmd = new SqlCommand())
            {
                ...
                SBSql.AppendLine(" FROM FAQ_Block Base ");
                SBSql.AppendLine(" WHERE (Base.FAQ_ID = @ParentID) AND (Base.Block_ID = @DataID)");
    ...
    protected void btn_BlockSave_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            using (SqlCommand cmd = new SqlCommand())
            {
                ...
                //[SQL] - 資料更新
                SBSql.AppendLine(" UPDATE FAQ_Block ");
                SBSql.AppendLine(" SET Block_Desc = @Block_Desc, Block_Pic = @Block_Pic, Sort = @Sort");
                SBSql.AppendLine(" WHERE (FAQ_ID = @ParentID) AND (Block_ID = @DataID)");
    ...
    protected void lbtn_DelFile_Click(object sender, EventArgs e)
    {
        try
        {
            using (SqlCommand cmd = new SqlCommand())
            {
                ...
                //[SQL] - 資料更新
                SBSql.AppendLine(" UPDATE FAQ_Block SET Block_Pic = NULL WHERE (FAQ_ID = @ParentID) AND (Block_ID = @DataID)");
    ...
    public string Page_CurrentUrl
    {
        get
        {
            return "{0}QA/Edit/DetailBox/{1}/{2}".FormatThis(
                 Application["WebUrl"]
                 , Param_parentID
