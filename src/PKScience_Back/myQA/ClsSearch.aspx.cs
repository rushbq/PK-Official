...
public partial class ClsSearch : SecurityCheck
{
    ...
    private void LookupDataList(int pageIndex)
    {
        ...
        {
            //[參數宣告] - 設定本頁Url(末端無須加 "/")
            this.ViewState["Page_Url"] = Application["WebUrl"] + "QAClass/Search";
            ...
            SBSql.AppendLine("      , (SELECT COUNT(*) FROM FAQ_Group WHERE (Class_ID = Base.Class_ID)) AS RelCnt");
            SBSql.AppendLine("      , ROW_NUMBER() OVER (ORDER BY Base.Sort, Base.Display DESC) AS RowRank ");
            SBSql.AppendLine("    FROM FAQ_Class Base ");
            SBSql.AppendLine("    WHERE (LOWER(Base.LangCode) = LOWER('zh-tw')) ");

            #region "..查詢條件.."
            //[查詢條件] - 關鍵字
            if (!string.IsNullOrEmpty(Req_Keyword))
            {
                SBSql.Append(" AND (LOWER(Base.Class_Name) LIKE LOWER('%' + @Keyword + '%'))");
        ...
        try
        {
            if (e.Item.ItemType == ListViewItemType.DataItem)
            {
                ...
                using (SqlCommand cmd = new SqlCommand())
                {
                    ...
                    SBSql.AppendLine(" DELETE FROM FAQ_Class WHERE (Class_ID = @DataID); ");
    ...
    protected void lvDataList_ItemDataBound(object sender, ListViewItemEventArgs e)
    {
        try
        {
            if (e.Item.ItemType == ListViewItemType.DataItem)
            {
                ...
                #region >> 是否已被使用 <<
                //取得資料, 判斷狀態
                int GetCnt = Convert.ToInt32(DataBinder.Eval(dataItem.DataItem, "RelCnt"));

                LinkButton lbtn_Del = (LinkButton)e.Item.FindControl("lbtn_Del");
                lbtn_Del.Visible = GetCnt > 0 ? false : true;
    ...
    protected void btn_Search_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            SBUrl.Append("{0}QAClass/Search/?srh=1".FormatThis(Application["WebUrl"]));
