...
public partial class News_Edit : SecurityCheck
{
                    ...
                    for (int row = 0; row < DT.Rows.Count; row++)
                    {
                        ...
                        html.AppendLine("<a href=\"{0}\" class=\"{1}\"><i class=\"{2}\"></i>&nbsp;{3}</a>".FormatThis(
                                "{0}News/Edit/Detail/{1}/{2}/{3}".FormatThis(
                                        ...
                                        , string.IsNullOrEmpty(NewsID) ? "" : Cryptograph.MD5Encrypt(NewsID, fn_Param.DesKey)
                                    )
                                , urlCss
                                , iconCss
    ...
    private void Add_Data()
    {
        using (SqlCommand cmd = new SqlCommand())
        {
            ...
            SBSql.AppendLine("  @NewID, @Group_Name, @StartTime, @EndTime, @Display, @Sort, @onIndex");
    ...
    protected void lbtn_Delete_Click(object sender, EventArgs e)
    {
        try
        {
            using (SqlCommand cmd = new SqlCommand())
            {
                ...
                {
                    //刪除整個Folder
                    string fileUrl = @"{0}News\{1}\".FormatThis(fn_Param.File_DiskUrl, Param_thisID);
