...
public partial class News_Search : SecurityCheck
{
    ...
    private void LookupDataList(int pageIndex)
    {
        ...
        try
        {
            ...
            SBSql.AppendLine("      , ROW_NUMBER() OVER (ORDER BY Base.Display DESC, Base.StartTime DESC, Base.Sort) AS RowRank ");
        ...
        try
        {
            if (e.Item.ItemType == ListViewItemType.DataItem)
            {
                ...
                using (SqlCommand cmd = new SqlCommand())
                {
                    ...
                    {
                        //刪除整個Folder
                        string fileUrl = @"{0}News\{1}\".FormatThis(fn_Param.File_DiskUrl, Get_DataID);
