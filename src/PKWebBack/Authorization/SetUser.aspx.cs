...
public partial class Authorization_SetUser : SecurityCheck
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            ...
            {
                //[權限判斷]
                if (fn_CheckAuth.CheckAuth("9901", out ErrMsg) == false)
                {
                ...
                //** 設定程式編號(重要) **
                if (false == setProgIDs.setID(this.Master, "9900", "9901"))
                {
                    throw new Exception("目前的Masterpage 沒有實作 IProgID,無法傳值");
                }

                //[取得/檢查參數] - 取得部門
                if (fn_CustomUI.Get_DeptList(this.ddl_Dept, "", true, out ErrMsg) == false)
                {
    ...
    private void LookupData()
    {
        try
        {
            ...
            using (SqlCommand cmd = new SqlCommand())
            {
                ...
                //[SQL] - 資料查詢
                SBSql.AppendLine(" SELECT Guid, Display_Name, Account_Name, DeptID ");
                SBSql.AppendLine(" FROM User_Profile ");
                SBSql.AppendLine(" WHERE (Account_Name = @DataID) ");
                ...
                using (DataTable DT = dbConn.LookupDT(cmd, dbConn.DBS.PKSYS, out ErrMsg))
                {
                    ...
                    else
                    {
                        ...
                        this.tb_EmpValue.Text = DT.Rows[0]["Account_Name"].ToString();
    ...
    {
        Response.Redirect("{0}Auth/User/Set/{1}/".FormatThis(
                    Application["WebUrl"]
                    , HttpUtility.UrlEncode(Cryptograph.MD5Encrypt(this.tb_EmpValue.Text, Application["DesKey"].ToString()))
                ));
    }
    ...
    protected void lbtn_Remove_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            using (SqlCommand cmd = new SqlCommand())
            {
                ...
                {
                    //[XML設定] - 使用者
                    //if (fn_CustomUI.DelXmlAuthProfile(Param_Guid, out ErrMsg) == false)
                    //{
                    //    fn_Extensions.JsAlert("使用者權限XML刪除失敗！", "");
                    //    return;
                    //}
                    //else
                    //{
    ...
    public string Page_CurrentUrl
    {
        get
        {
            return "{0}Auth/User/Set/{1}/".FormatThis(
                Application["WebUrl"]
