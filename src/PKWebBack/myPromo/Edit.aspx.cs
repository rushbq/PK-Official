...
public partial class Promo_Edit : SecurityCheck
{
    ...
    private void LookupData()
    {
        try
        {
            ...
            using (SqlCommand cmd = new SqlCommand())
            {
                ...
                SBSql.AppendLine("            LEFT JOIN Promo Base ON Lang.LangCode = Base.LangCode AND Base.Group_ID = GP.Group_ID ");
                SBSql.AppendLine("            WHERE (Lang.Display = 'Y') AND (Base.Promo_ID IS NULL) ");
    ...
    private void LookupData_Detail()
    {
        try
        {
            using (SqlCommand cmd = new SqlCommand())
            {
                ...
                SBSql.AppendLine(" SELECT Lang.LangName, Lang.LangCode, Base.Promo_ID ");
                SBSql.AppendLine("     , (CASE WHEN Base.Promo_ID IS NULL THEN 'N' ELSE 'Y' END) IsSet ");
                SBSql.AppendLine(" FROM PKSYS.dbo.Param_Language Lang ");
                SBSql.AppendLine("     LEFT JOIN Promo Base ON Lang.LangCode = Base.LangCode AND Base.Group_ID = @DataID ");
                ...
                using (DataTable DT = dbConn.LookupDT(cmd, out ErrMsg))
                {
                    ...
                    for (int row = 0; row < DT.Rows.Count; row++)
                    {
                        ...
                        string PromoID = DT.Rows[row]["Promo_ID"].ToString();
                        ...
                        html.AppendLine("<a href=\"{0}\" class=\"{1}\"><i class=\"{2}\"></i>&nbsp;{3}</a>".FormatThis(
                                "{0}Promo/Edit/Detail/{1}/{2}/{3}".FormatThis(
                                        Application["WebUrl"]
                                        , Cryptograph.MD5Encrypt(Param_thisID, Application["DesKey"].ToString())
                                        , DT.Rows[row]["LangCode"].ToString()
                                        , string.IsNullOrEmpty(PromoID) ? "" : Cryptograph.MD5Encrypt(PromoID, Application["DesKey"].ToString())
                                    )
                                , urlCss
                                , iconCss
                    ...
                    //輸出Html
                    this.lt_PromoSet.Text = html.ToString();
                }
            }
        }
        catch (Exception)
    ...
    private void Add_Data()
    {
        using (SqlCommand cmd = new SqlCommand())
        {
            ...
            SBSql.AppendLine("  SELECT ISNULL(MAX(Group_ID) ,0) + 1 FROM Promo_Group ");
            SBSql.AppendLine(" );");
            ...
            //[SQL] - 資料新增
            SBSql.AppendLine(" INSERT INTO Promo_Group( ");
            SBSql.AppendLine("  Group_ID, Group_Name, StartTime, EndTime, Display, Sort");
            ...
            if (GetArea.Count() > 0)
            {
                ...
                foreach (var AreaCode in GetArea)
                {
                    ...
                    SBSql.AppendLine(" INSERT INTO Promo_Area( ");
            ...
            //更新本頁Url
            string thisUrl = "{0}Promo/Edit/{1}/".FormatThis(
                        Application["WebUrl"]
    ...
    private void Edit_Data()
    {
        using (SqlCommand cmd = new SqlCommand())
        {
            ...
            //[SQL] - 資料更新
            SBSql.AppendLine(" UPDATE Promo_Group ");
            ...
            {
                //先清除資料
                SBSql.AppendLine(" DELETE FROM Promo_Area WHERE (Group_ID = @DataID);");
    ...
    protected void lbtn_Delete_Click(object sender, EventArgs e)
    {
        try
        {
            using (SqlCommand cmd = new SqlCommand())
            {
                ...
                //[SQL] - 資料更新
                SBSql.AppendLine(" DELETE FROM Promo_Block WHERE (Promo_ID IN (SELECT Promo_ID FROM Promo WHERE (Group_ID = @DataID))); ");
                SBSql.AppendLine(" DELETE FROM Promo WHERE (Group_ID = @DataID); ");
                SBSql.AppendLine(" DELETE FROM Promo_Area WHERE (Group_ID = @DataID); ");
                SBSql.AppendLine(" DELETE FROM Promo_Group WHERE (Group_ID = @DataID); ");
                ...
                {
                    //刪除整個Folder
                    string fileUrl = @"{0}Promo\{1}\".FormatThis(Application["File_DiskUrl"], Param_thisID);
    ...
    public string Page_SearchUrl
    {
        get
        {
            ...
            {
                Url = "{0}Promo/Search/".FormatThis(Application["WebUrl"]);
            }
            else
            {
