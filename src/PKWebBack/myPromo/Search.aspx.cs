...
public partial class Promo_Search : SecurityCheck
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            if (!IsPostBack)
            {
                ...
                //[取得/檢查參數] - 區域
                if (fn_CustomUI.Get_Area(this.cbl_Area, Req_Area, out ErrMsg) == false)
                {
    ...
    private void LookupDataList(int pageIndex)
    {
        ...
        {
            //[參數宣告] - 設定本頁Url(末端無須加 "/")
            this.ViewState["Page_Url"] = Application["WebUrl"] + "Promo/Search";
            ...
            SBSql.AppendLine("      Base.Group_ID, Base.Group_Name, Base.StartTime, Base.EndTime, Base.Display ");
            SBSql.AppendLine("      , ROW_NUMBER() OVER (ORDER BY Base.Display DESC, Base.Group_ID DESC) AS RowRank ");
            SBSql.AppendLine("    FROM Promo_Group Base ");
            ...
            if (Req_Area != null)
            {
                ...
                SBSql.Append(" AND (Base.Group_ID IN (SELECT Area.Group_ID FROM Promo_Area Area WHERE (Area.AreaCode IN ({0})))) ".FormatThis(
        ...
        try
        {
            if (e.Item.ItemType == ListViewItemType.DataItem)
            {
                ...
                using (SqlCommand cmd = new SqlCommand())
                {
                    //刪除資料
                    StringBuilder SBSql = new StringBuilder();
                    SBSql.AppendLine(" DELETE FROM Promo_Block WHERE (Promo_ID IN (SELECT Promo_ID FROM Promo WHERE (Group_ID = @Param_ID))); ");
                    SBSql.AppendLine(" DELETE FROM Promo WHERE (Group_ID = @Param_ID); ");
                    SBSql.AppendLine(" DELETE FROM Promo_Area WHERE (Group_ID = @Param_ID); ");
                    SBSql.AppendLine(" DELETE FROM Promo_Group WHERE (Group_ID = @Param_ID); ");
                    ...
                    {
                        //刪除整個Folder
                        string fileUrl = @"{0}Promo\{1}\".FormatThis(Application["File_DiskUrl"], Get_DataID);
    ...
    protected void btn_Search_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            SBUrl.Append("{0}Promo/Search/?srh=1".FormatThis(Application["WebUrl"]));
