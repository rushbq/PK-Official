...
public partial class News_Edit_Sub : SecurityCheck
{
    ...
    private void LookupData()
    {
        try
        {
            ...
            using (SqlCommand cmd = new SqlCommand())
            {
                ...
                //[SQL] - 資料查詢
                SBSql.AppendLine(" SELECT Base.News_ID, Base.News_Title, Base.News_Desc, Base.News_Pic, Base.News_PubDate");
                ...
                using (DataTable DT = dbConn.LookupDT(cmd, out ErrMsg))
                {
                    ...
                    {
                        //[填入資料]
                        this.show_sDate.Text = DT.Rows[0]["News_PubDate"].ToString().ToDateString("yyyy-MM-dd");
                        this.tb_News_PubDate.Text = DT.Rows[0]["News_PubDate"].ToString().ToDateString("yyyy-MM-dd");
                        this.tb_News_Title.Text = DT.Rows[0]["News_Title"].ToString();
                        this.tb_News_Desc.Text = DT.Rows[0]["News_Desc"].ToString();

                        //檔案Url
                        string myFile = DT.Rows[0]["News_Pic"].ToString();
    ...
    private void LookupData_Block()
    {
        try
        {
            using (SqlCommand cmd = new SqlCommand())
            {
                ...
                SBSql.AppendLine(" FROM News_Block ");
                SBSql.AppendLine(" WHERE (News_ID = @DataID) ");
        ...
        try
        {
            if (e.Item.ItemType == ListViewItemType.DataItem)
            {
                using (SqlCommand cmd = new SqlCommand())
                {
                    ...
                    //[SQL] - 刪除資料
                    SBSql.AppendLine(" DELETE FROM News_Block WHERE (News_ID = @DataID) AND (Block_ID = @Param_ID) ");
                    cmd.CommandText = SBSql.ToString();
                    cmd.Parameters.Clear();
                    cmd.Parameters.AddWithValue("DataID", Param_thisID);
                    ...
                    else
                    {
                        ...
                        fn_Log.Log_Rec("後台"
                            , "刪除News區塊"
                            , "Group ID:{0}, News ID:{1}, Block ID:{2}".FormatThis(Param_parentID, Param_thisID, GetDataID)
                            , Session["Login_UserID"].ToString());
    ...
    protected void btn_Save_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            //[參數檢查] - 必填項目
            if (fn_Extensions.String_資料長度Byte(this.tb_News_Title.Text, "1", "150", out ErrMsg) == false)
            {
                SBAlert.Append("「標題」請輸入1 ~ 70個字\\n");
            }
            //[參數檢查] - 必填項目
            if (fn_Extensions.String_資料長度Byte(this.tb_News_Desc.Text, "1", "500", out ErrMsg) == false)
            {
    ...
    private void Add_Data(List<TempParam> ITempList)
    {
        using (SqlCommand cmd = new SqlCommand())
        {
            ...
            SBSql.AppendLine("  SELECT ISNULL(MAX(News_ID) ,0) + 1 FROM News ");
            SBSql.AppendLine(" );");
            ...
            //[SQL] - 資料新增
            SBSql.AppendLine(" INSERT INTO News( ");
            SBSql.AppendLine("  Group_ID, News_ID, LangCode");
            SBSql.AppendLine("  , News_Title, News_Desc, News_Pic, News_PubDate");
            ...
            cmd.Parameters.AddWithValue("LangCode", Param_LangCode);
            cmd.Parameters.AddWithValue("News_Title", this.tb_News_Title.Text.Trim());
            cmd.Parameters.AddWithValue("News_Desc", this.tb_News_Desc.Text);
            cmd.Parameters.AddWithValue("News_Pic", (ITempList.Count == 0) ? "" : ITempList[0].Param_Pic);
            cmd.Parameters.AddWithValue("News_PubDate", this.tb_News_PubDate.Text.ToDateString("yyyy-MM-dd"));
            ...
            //更新本頁Url
            string thisUrl = "{0}News/Edit/Detail/{1}/{2}/{3}/".FormatThis(
                        Application["WebUrl"]
    ...
    private void Edit_Data(List<TempParam> ITempList)
    {
        using (SqlCommand cmd = new SqlCommand())
        {
            ...
            //[SQL] - 資料更新
            SBSql.AppendLine(" UPDATE News ");
            SBSql.AppendLine(" SET News_Title = @News_Title ");
            SBSql.AppendLine("  , News_Desc = @News_Desc, News_Pic = @News_Pic, News_PubDate = @News_PubDate");
            ...
            cmd.Parameters.AddWithValue("News_Title", this.tb_News_Title.Text.Trim());
            cmd.Parameters.AddWithValue("News_Desc", this.tb_News_Desc.Text);
            cmd.Parameters.AddWithValue("News_Pic", (ITempList.Count == 0) ? this.hf_OldFile.Value : ITempList[0].Param_Pic);
            cmd.Parameters.AddWithValue("News_PubDate", this.tb_News_PubDate.Text.ToDateString("yyyy-MM-dd"));
    ...
    protected void lbtn_DelFile_Click(object sender, EventArgs e)
    {
        try
        {
            using (SqlCommand cmd = new SqlCommand())
            {
                ...
                //[SQL] - 資料更新
                SBSql.AppendLine(" UPDATE News SET News_Pic = NULL WHERE (News_ID = @DataID) ");
    ...
    protected void btn_BlockSave_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            using (SqlCommand cmd = new SqlCommand())
            {
                ...
                SBSql.AppendLine(" DECLARE @NewID AS INT ");
                SBSql.AppendLine(" SET @NewID = (");
                SBSql.AppendLine("  SELECT ISNULL(MAX(Block_ID) ,0) + 1 FROM News_Block WHERE (News_ID = @News_ID)");
                SBSql.AppendLine(" );");
                SBSql.AppendLine(" SELECT @NewID AS NewID");

                cmd.CommandText = SBSql.ToString();
                cmd.Parameters.AddWithValue("News_ID", Param_thisID);
                using (DataTable DT = dbConn.LookupDT(cmd, out ErrMsg))
                {
                    NewID = Convert.ToInt32(DT.Rows[0]["NewID"]);
                }
                ...
                SBSql.Append(" INSERT INTO News_Block( ");
                SBSql.Append("  News_ID, Block_ID");
                ...
                SBSql.Append(" UPDATE News SET ");
                SBSql.Append(" Update_Who = @Update_Who, Update_Time = GETDATE() ");
                SBSql.Append(" WHERE (Group_ID = @Group_ID) AND (News_ID = @News_ID) ");
                ...
                fn_Log.Log_Rec("後台"
                    , "新增News區塊"
                    , "Group ID:{0}, News ID:{1}, Block ID:{2}".FormatThis(Param_parentID, Param_thisID, NewID.ToString())
                    , Session["Login_UserID"].ToString());
        ...
        try
        {
            ...
            using (SqlCommand cmd = new SqlCommand())
            {
                ...
                for (int row = 0; row < lvDataList.Items.Count; row++)
                {
                    ...
                    SBSql.AppendLine(" UPDATE News_Block SET Sort = @lvParam_Sort_{0}".FormatThis(row));
                    SBSql.AppendLine(" WHERE (News_ID = @News_ID) AND (Block_ID = @lvParam_ID_{0}) ".FormatThis(row));
    ...
    public string Page_CurrentUrl
    {
        get
        {
            return "{0}News/Edit/Detail/{1}/{2}/{3}/".FormatThis(
                Application["WebUrl"]
                , Cryptograph.MD5Encrypt(Param_parentID, Application["DesKey"].ToString())
                , Param_LangCode
    ...
    public string Param_FileFolder
    {
        get
        {
            return this._Param_FileFolder != null
                ...
                : @"{0}News\{1}\".FormatThis(Application["File_DiskUrl"], Param_parentID);
        }
        set
        {
    ...
    public string Param_WebFolder
    {
        get
        {
            return this._Param_WebFolder != null
                ...
                : @"{0}News/{1}/".FormatThis(Application["File_WebUrl"], Param_parentID);
        }
        set
        {
