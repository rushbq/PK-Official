...
public partial class Expo_Edit : SecurityCheck
{
    ...
    private void LookupData()
    {
        try
        {
            ...
            using (SqlCommand cmd = new SqlCommand())
            {
                ...
                SBSql.AppendLine("            LEFT JOIN Expo Base ON Lang.LangCode = Base.LangCode AND Base.Group_ID = GP.Group_ID ");
                SBSql.AppendLine("            WHERE (Lang.Display = 'Y') AND (Base.Expo_ID IS NULL) ");
                ...
                using (DataTable DT = dbConn.LookupDT(cmd, out ErrMsg))
                {
                    ...
                    for (int row = 0; row < DT.Rows.Count; row++)
                    {
                        ...
                        html.AppendLine("<a href=\"{0}\" class=\"{1}\"><i class=\"{2}\"></i>&nbsp;{3}</a>".FormatThis(
                                "{0}Expo/Edit/Detail/{1}/{2}/{3}".FormatThis(
                                        Application["WebUrl"]
                                        , Cryptograph.MD5Encrypt(Param_thisID, Application["DesKey"].ToString())
                                        , DT.Rows[row]["LangCode"].ToString()
                                        , string.IsNullOrEmpty(ExpoID) ? "" : Cryptograph.MD5Encrypt(ExpoID, Application["DesKey"].ToString())
                                    )
                                , urlCss
                                , iconCss
        ...
        using (SqlCommand cmd = new SqlCommand())
        {
            ...
            if (GetArea.Count() > 0)
            {
                ...
                foreach (var AreaCode in GetArea)
                {
                    ...
                    SBSql.AppendLine(" INSERT INTO Expo_Area( ");
            ...
            //更新本頁Url
            string thisUrl = "{0}Expo/Edit/{1}/".FormatThis(
                        Application["WebUrl"]
    ...
    private void Edit_Data()
    {
        using (SqlCommand cmd = new SqlCommand())
        {
            ...
            {
                //先清除資料
                SBSql.AppendLine(" DELETE FROM Expo_Area WHERE (Group_ID = @DataID);");
    ...
    protected void lbtn_Delete_Click(object sender, EventArgs e)
    {
        try
        {
            using (SqlCommand cmd = new SqlCommand())
            {
                ...
                //[SQL] - 資料更新
                SBSql.AppendLine(" DELETE FROM Expo_Photos WHERE (Expo_ID IN (SELECT Expo_ID FROM Expo WHERE (Group_ID = @DataID))); ");
                ...
                {
                    //刪除整個Folder
                    string fileUrl = @"{0}Expo\{1}\".FormatThis(Application["File_DiskUrl"], Param_thisID);
    ...
    public string Page_SearchUrl
    {
        get
        {
            ...
            {
                Url = "{0}Expo/Search/".FormatThis(Application["WebUrl"]);
            }
            else
            {
