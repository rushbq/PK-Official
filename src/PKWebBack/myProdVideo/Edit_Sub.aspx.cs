...
public partial class PV_Edit_Sub : SecurityCheck
{
    ...
    private void LookupData()
    {
        try
        {
            ...
            using (SqlCommand cmd = new SqlCommand())
            {
                ...
                //[SQL] - 資料查詢
                SBSql.AppendLine(" SELECT Base.PV_ID, Base.PV_Title, Base.PV_PubDate, Base.PV_Pic, Base.PV_UriSrc, Base.PV_Uri");
                SBSql.AppendLine(" FROM PV Base ");
                SBSql.AppendLine(" WHERE (Base.PV_ID = @DataID)");
                cmd.CommandText = SBSql.ToString();
                ...
                using (DataTable DT = dbConn.LookupDT(cmd, out ErrMsg))
                {
                    ...
                    {
                        //[填入資料]
                        this.tb_PV_Title.Text = DT.Rows[0]["PV_Title"].ToString();
                        this.show_sDate.Text = DT.Rows[0]["PV_PubDate"].ToString().ToDateString("yyyy-MM-dd");
                        this.tb_PV_PubDate.Text = DT.Rows[0]["PV_PubDate"].ToString().ToDateString("yyyy-MM-dd");
                        this.rbl_PV_UriSrc.SelectedValue = DT.Rows[0]["PV_UriSrc"].ToString();
                        this.tb_PV_Uri.Text = DT.Rows[0]["PV_Uri"].ToString();

                        //檔案Url - 小圖
                        string myFile = DT.Rows[0]["PV_Pic"].ToString();
    ...
    protected void btn_Save_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            //[參數檢查] - 必填項目
            if (fn_Extensions.String_資料長度Byte(this.tb_PV_Title.Text, "1", "200", out ErrMsg) == false)
            {
                SBAlert.Append("「標題」請輸入1 ~ 100個字\\n");
            }
    ...
    private void Add_Data(List<TempParam> ITempList)
    {
        using (SqlCommand cmd = new SqlCommand())
        {
            ...
            SBSql.AppendLine("  SELECT ISNULL(MAX(PV_ID) ,0) + 1 FROM PV ");
            SBSql.AppendLine(" );");
            ...
            //[SQL] - 資料新增
            SBSql.AppendLine(" INSERT INTO PV( ");
            SBSql.AppendLine("  Group_ID, PV_ID, LangCode");
            SBSql.AppendLine("  , PV_Title, PV_PubDate, PV_Pic, PV_UriSrc, PV_Uri");
            ...
            cmd.Parameters.AddWithValue("LangCode", Param_LangCode);
            cmd.Parameters.AddWithValue("PV_Title", this.tb_PV_Title.Text.Trim());
            cmd.Parameters.AddWithValue("PV_PubDate", this.tb_PV_PubDate.Text.ToDateString("yyyy-MM-dd"));
            cmd.Parameters.AddWithValue("PV_Pic", pic1);
            cmd.Parameters.AddWithValue("PV_UriSrc", this.rbl_PV_UriSrc.SelectedValue);
            cmd.Parameters.AddWithValue("PV_Uri", this.tb_PV_Uri.Text);
            ...
            //更新本頁Url
            string thisUrl = "{0}PV/Edit/Detail/{1}/{2}/{3}/".FormatThis(
                        Application["WebUrl"]
    ...
    private void Edit_Data(List<TempParam> ITempList)
    {
        using (SqlCommand cmd = new SqlCommand())
        {
            ...
            //[SQL] - 資料更新
            SBSql.AppendLine(" UPDATE PV ");
            SBSql.AppendLine(" SET PV_Title = @PV_Title ");
            SBSql.AppendLine("  , PV_PubDate = @PV_PubDate, PV_Pic = @PV_Pic, PV_UriSrc = @PV_UriSrc, PV_Uri = @PV_Uri");
            SBSql.AppendLine("  , Update_Who = @Update_Who, Update_Time = GETDATE() ");
            SBSql.AppendLine(" WHERE (PV_ID = @DataID) ");
    ...
    protected void lbtn_DelFile_Click(object sender, EventArgs e)
    {
        try
        {
            using (SqlCommand cmd = new SqlCommand())
            {
                ...
                //[SQL] - 資料更新
                SBSql.AppendLine(" UPDATE PV SET PV_Pic = NULL WHERE (PV_ID = @DataID) ");
    ...
    public string Page_CurrentUrl
    {
        get
        {
            return "{0}PV/Edit/Detail/{1}/{2}/{3}/".FormatThis(
                Application["WebUrl"]
                , Cryptograph.MD5Encrypt(Param_parentID, Application["DesKey"].ToString())
                , Param_LangCode
    ...
    public string Param_FileFolder
    {
        get
        {
            return this._Param_FileFolder != null
                ...
                : @"{0}PV\{1}\".FormatThis(Application["File_DiskUrl"], Param_parentID);
        }
        set
        {
    ...
    public string Param_WebFolder
    {
        get
        {
            return this._Param_WebFolder != null
                ...
                : @"{0}PV/{1}/".FormatThis(Application["File_WebUrl"], Param_parentID);
        }
        set
        {
