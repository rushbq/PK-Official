...
public partial class PV_Search : SecurityCheck
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            if (!IsPostBack)
            {
                ...
                //[取得/檢查參數] - ClassType
                if (!string.IsNullOrEmpty(Req_ClassType))
                {
                    this.ddl_ClassType.SelectedValue = Req_ClassType;
                }
    ...
    private void LookupDataList(int pageIndex)
    {
        ...
        {
            //[參數宣告] - 設定本頁Url(末端無須加 "/")
            this.ViewState["Page_Url"] = Application["WebUrl"] + "PV/Search";
            ...
            SBSql.AppendLine("      Base.Group_ID, Base.Group_Name, Base.StartTime, Base.EndTime, Base.Display, Base.ClassType ");
            SBSql.AppendLine("      , ROW_NUMBER() OVER (ORDER BY Base.Group_ID DESC, Base.Display DESC) AS RowRank ");
            SBSql.AppendLine("    FROM PV_Group Base ");
            ...
            if (!string.IsNullOrEmpty(Req_ClassType))
            {
                SBSql.Append(" AND (Base.ClassType = @ClassType) ");
                cmd.Parameters.AddWithValue("ClassType", Req_ClassType);

                Params.Add("type=" + Server.UrlEncode(Req_ClassType));
            }
            ...
            //    SBSql.Append(" AND (Base.Group_ID IN (SELECT Area.Group_ID FROM PV_Area Area WHERE (Area.AreaCode IN ({0})))) ".FormatThis(
            ...
            {
                SBSql.Append(" AND (Base.ClassType = @ClassType) ");
                cmdTotalCnt.Parameters.AddWithValue("ClassType", Req_ClassType);
            }
        ...
        try
        {
            if (e.Item.ItemType == ListViewItemType.DataItem)
            {
                ...
                using (SqlCommand cmd = new SqlCommand())
                {
                    ...
                    {
                        //刪除整個Folder
                        string fileUrl = @"{0}PV\{1}\".FormatThis(Application["File_DiskUrl"], Get_DataID);
    ...
    protected void lvDataList_ItemDataBound(object sender, ListViewItemEventArgs e)
    {
        try
        {
            if (e.Item.ItemType == ListViewItemType.DataItem)
            {
                ...
                //取得資料, 判斷區塊
                string GetClsType = DataBinder.Eval(dataItem.DataItem, "ClassType").ToString();
                Label lb_Type = (Label)e.Item.FindControl("lb_Type");
                lb_Type.Text = GetClsType.Equals("A") ? "工具" : "玩具";
    ...
    protected void btn_Search_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            SBUrl.Append("{0}PV/Search/?srh=1".FormatThis(Application["WebUrl"]));
            ...
            {
                SBUrl.Append("&type=" + Server.UrlEncode(this.ddl_ClassType.SelectedValue));
            }
