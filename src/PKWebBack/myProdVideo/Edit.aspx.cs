...
public partial class PV_Edit : SecurityCheck
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            ...
            {
                //[權限判斷]
                if (fn_CheckAuth.CheckAuth("420", out ErrMsg) == false)
                {
                ...
                //** 設定程式編號(重要) **
                if (false == setProgIDs.setID(this.Master, "400", "420"))
                {
                ...
                {
                    //default value
                    //show_sDate.Text = DateTime.Now.ToString().ToDateString("yyyy/MM/dd HH:mm");
                    //tb_StartDate.Text = DateTime.Now.ToString().ToDateString("yyyy/MM/dd HH:mm");
                }
    ...
    private void LookupData()
    {
        try
        {
            ...
            using (SqlCommand cmd = new SqlCommand())
            {
                ...
                SBSql.AppendLine("            LEFT JOIN PV Base ON Lang.LangCode = Base.LangCode AND Base.Group_ID = GP.Group_ID ");
                SBSql.AppendLine("            WHERE (Lang.Display = 'Y') AND (Base.PV_ID IS NULL) ");
                ...
                using (DataTable DT = dbConn.LookupDT(cmd, out ErrMsg))
                {
                    ...
                    else
                    {
                        ...
                        this.rbl_ClassType.SelectedValue = DT.Rows[0]["ClassType"].ToString();
    ...
    private void LookupData_Detail()
    {
        try
        {
            using (SqlCommand cmd = new SqlCommand())
            {
                ...
                SBSql.AppendLine(" SELECT Lang.LangName, Lang.LangCode, Base.PV_ID ");
                SBSql.AppendLine("     , (CASE WHEN Base.PV_ID IS NULL THEN 'N' ELSE 'Y' END) IsSet ");
                SBSql.AppendLine(" FROM PKSYS.dbo.Param_Language Lang ");
                SBSql.AppendLine("     LEFT JOIN PV Base ON Lang.LangCode = Base.LangCode AND Base.Group_ID = @DataID ");
                ...
                using (DataTable DT = dbConn.LookupDT(cmd, out ErrMsg))
                {
                    ...
                    for (int row = 0; row < DT.Rows.Count; row++)
                    {
                        ...
                        string PVID = DT.Rows[row]["PV_ID"].ToString();
                        ...
                        html.AppendLine("<a href=\"{0}\" class=\"{1}\"><i class=\"{2}\"></i>&nbsp;{3}</a>".FormatThis(
                                "{0}PV/Edit/Detail/{1}/{2}/{3}".FormatThis(
                                        Application["WebUrl"]
                                        , Cryptograph.MD5Encrypt(Param_thisID, Application["DesKey"].ToString())
                                        , DT.Rows[row]["LangCode"].ToString()
                                        , string.IsNullOrEmpty(PVID) ? "" : Cryptograph.MD5Encrypt(PVID, Application["DesKey"].ToString())
                                    )
                                , urlCss
                                , iconCss
    ...
    private void LookupData_Rel_ModelNo()
    {
        try
        {
            using (SqlCommand cmd = new SqlCommand())
            {
                ...
                SBSql.AppendLine(" FROM PV_Group_Rel_ModelNo ");
    ...
    private void Add_Data()
    {
        using (SqlCommand cmd = new SqlCommand())
        {
            ...
            SBSql.AppendLine("  SELECT ISNULL(MAX(Group_ID) ,0) + 1 FROM PV_Group ");
            SBSql.AppendLine(" );");
            ...
            //[SQL] - 資料新增
            SBSql.AppendLine(" INSERT INTO PV_Group( ");
            SBSql.AppendLine("  Group_ID, Group_Name, StartTime, EndTime, ClassType, Display, Sort");
            SBSql.AppendLine("  , Create_Who, Create_Time");
            SBSql.AppendLine(" ) VALUES ( ");
            //SBSql.AppendLine("  @NewID, @Group_Name, @StartTime, @EndTime, 'S', @Sort");
            SBSql.AppendLine("  @NewID, @Group_Name, NULL, NULL, @ClassType, @Display, @Sort");
            ...
            if (GetArea.Count() > 0)
            {
                ...
                foreach (var AreaCode in GetArea)
                {
                    ...
                    SBSql.AppendLine(" INSERT INTO PV_Area( ");
            ...
            cmd.Parameters.AddWithValue("ClassType", this.rbl_ClassType.SelectedValue);
            ...
            //更新本頁Url
            string thisUrl = "{0}PV/Edit/{1}/".FormatThis(
                        Application["WebUrl"]
            ...
            {
                fn_Extensions.JsAlert("品號設定失敗！", thisUrl);
                return;
            }
            else
            {
    ...
    private void Edit_Data()
    {
        using (SqlCommand cmd = new SqlCommand())
        {
            ...
            //[SQL] - 資料更新
            SBSql.AppendLine(" UPDATE PV_Group ");
            SBSql.AppendLine(" SET Group_Name = @Group_Name ");
            //SBSql.AppendLine("  , StartTime = @StartTime, EndTime = @EndTime");
            SBSql.AppendLine("  , ClassType = @ClassType, Display = @Display, Sort = @Sort");
            ...
            {
                //先清除資料
                SBSql.AppendLine(" DELETE FROM PV_Area WHERE (Group_ID = @DataID);");
            ...
            {
                fn_Extensions.JsAlert("品號設定失敗！", Page_CurrentUrl);
                return;
            }
            else
            {
    ...
    protected void lbtn_Delete_Click(object sender, EventArgs e)
    {
        try
        {
            using (SqlCommand cmd = new SqlCommand())
            {
                ...
                //[SQL] - 資料更新
                SBSql.AppendLine(" DELETE FROM PV_Group_Rel_ModelNo WHERE (Group_ID = @DataID); ");
                SBSql.AppendLine(" DELETE FROM PV WHERE (Group_ID = @DataID); ");
                SBSql.AppendLine(" DELETE FROM PV_Area WHERE (Group_ID = @DataID); ");
                SBSql.AppendLine(" DELETE FROM PV_Group WHERE (Group_ID = @DataID); ");
                ...
                {
                    //刪除整個Folder
                    string fileUrl = @"{0}PV\{1}\".FormatThis(Application["File_DiskUrl"], Param_thisID);
    ...
    private bool Set_DataRel(string DataID)
    {
        ...
        using (SqlCommand cmd = new SqlCommand())
        {
            ...
            foreach (var item in query)
            {
                ...
                SBSql.AppendLine(" INSERT INTO PV_Group_Rel_ModelNo( ");
    ...
    public string Page_SearchUrl
    {
        get
        {
            ...
            {
                Url = "{0}PV/Search/".FormatThis(Application["WebUrl"]);
            }
            else
            {
