...
public partial class Edit_DetailBox : SecurityCheck
{
    ...
    private void LookupData()
    {
        try
        {
            ...
            using (SqlCommand cmd = new SqlCommand())
            {
                ...
                SBSql.AppendLine(" FROM Article_Block Base ");
                SBSql.AppendLine(" WHERE (Base.Article_ID = @ParentID) AND (Base.Block_ID = @DataID)");
    ...
    protected void btn_BlockSave_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            using (SqlCommand cmd = new SqlCommand())
            {
                ...
                //[SQL] - 資料更新
                SBSql.AppendLine(" UPDATE Article_Block ");
                SBSql.AppendLine(" SET Block_Desc = @Block_Desc, Block_Pic = @Block_Pic, Sort = @Sort");
                SBSql.AppendLine(" WHERE (Article_ID = @ParentID) AND (Block_ID = @DataID)");
    ...
    protected void lbtn_DelFile_Click(object sender, EventArgs e)
    {
        try
        {
            using (SqlCommand cmd = new SqlCommand())
            {
                ...
                //[SQL] - 資料更新
                SBSql.AppendLine(" UPDATE Article_Block SET Block_Pic = NULL WHERE (Article_ID = @ParentID) AND (Block_ID = @DataID)");
    ...
    public string Page_CurrentUrl
    {
        get
        {
            return "{0}Article/Edit/DetailBox/{1}/{2}/{3}".FormatThis(
                 Application["WebUrl"]
                 , Param_GID
                 , Param_parentID
    ...
    public string Param_FileFolder
    {
        get
        {
            return this._Param_FileFolder != null
                ...
                : @"{0}Article\{1}\".FormatThis(fn_Param.File_DiskUrl, Param_GID);
        }
        set
        {
    ...
    public string Param_WebFolder
    {
        get
        {
            return this._Param_WebFolder != null
                ...
                : @"{0}Article/{1}/".FormatThis(fn_Param.File_WebUrl, Param_GID);
        }
        set
        {
