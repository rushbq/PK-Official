...
public partial class Article_Edit_Sub : SecurityCheck
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            ...
            {
                //[權限判斷]
                if (fn_CheckAuth.CheckAuth("130", out ErrMsg) == false)
                {
                ...
                //** 設定程式編號(重要) **
                if (false == setProgIDs.setID(this.Master, "100", "130"))
                {
    ...
    private void LookupData()
    {
        try
        {
            ...
            using (SqlCommand cmd = new SqlCommand())
            {
                ...
                //[SQL] - 資料查詢
                SBSql.AppendLine(" SELECT Base.Article_ID, Base.Article_Title, Base.Article_Desc, Base.Article_Pic, Base.Article_PubDate");
                ...
                SBSql.AppendLine("  , (SELECT Display_Name FROM {0}.dbo.User_Profile WHERE ([Guid] = Base.Update_Who)) AS Update_Name "
                    .FormatThis(fn_SysDB.Param_DB));
                SBSql.AppendLine(" FROM Article Base ");
                SBSql.AppendLine(" WHERE (Base.Article_ID = @DataID)");
                cmd.CommandText = SBSql.ToString();
                ...
                using (DataTable DT = dbConn.LookupDT(cmd, out ErrMsg))
                {
                    ...
                    {
                        //[填入資料]
                        this.show_sDate.Text = DT.Rows[0]["Article_PubDate"].ToString().ToDateString("yyyy-MM-dd");
                        this.tb_Article_PubDate.Text = DT.Rows[0]["Article_PubDate"].ToString().ToDateString("yyyy-MM-dd");
                        this.tb_Article_Title.Text = DT.Rows[0]["Article_Title"].ToString();
                        this.tb_Article_Desc.Text = DT.Rows[0]["Article_Desc"].ToString();

                        //檔案Url
                        string myFile = DT.Rows[0]["Article_Pic"].ToString();
                        this.hf_OldFile.Value = myFile;
                        if (!string.IsNullOrEmpty(myFile))
                        {
                            this.lt_dwUrl.Text = "<a href=\"{0}\" class=\"btn btn-info zoomPic\" title=\"查看圖片\"><i class=\"fa fa-eye\"></i></a>".FormatThis(
    ...
    private void LookupData_Block()
    {
        try
        {
            using (SqlCommand cmd = new SqlCommand())
            {
                ...
                SBSql.AppendLine(" FROM Article_Block ");
                SBSql.AppendLine(" WHERE (Article_ID = @DataID) ");
        ...
        try
        {
            if (e.Item.ItemType == ListViewItemType.DataItem)
            {
                using (SqlCommand cmd = new SqlCommand())
                {
                    ...
                    //[SQL] - 刪除資料
                    SBSql.AppendLine(" DELETE FROM Article_Block WHERE (Article_ID = @DataID) AND (Block_ID = @Param_ID) ");
                    cmd.CommandText = SBSql.ToString();
                    cmd.Parameters.Clear();
                    cmd.Parameters.AddWithValue("DataID", Param_thisID);
    ...
    protected void btn_Save_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            //[參數檢查] - 必填項目
            if (fn_Extensions.String_資料長度Byte(this.tb_Article_Title.Text, "1", "150", out ErrMsg) == false)
            {
                SBAlert.Append("「標題」請輸入1 ~ 70個字\\n");
            }
            //[參數檢查] - 必填項目
            if (fn_Extensions.String_資料長度Byte(this.tb_Article_Desc.Text, "1", "500", out ErrMsg) == false)
            {
    ...
    private void Add_Data(List<TempParam> ITempList)
    {
        using (SqlCommand cmd = new SqlCommand())
        {
            ...
            SBSql.AppendLine("  SELECT ISNULL(MAX(Article_ID) ,0) + 1 FROM Article ");
            SBSql.AppendLine(" );");
            ...
            //[SQL] - 資料新增
            SBSql.AppendLine(" INSERT INTO Article( ");
            SBSql.AppendLine("  Group_ID, Article_ID, LangCode");
            SBSql.AppendLine("  , Article_Title, Article_Desc, Article_Pic, Article_PubDate");
            ...
            cmd.Parameters.AddWithValue("LangCode", Param_LangCode);
            cmd.Parameters.AddWithValue("Article_Title", this.tb_Article_Title.Text.Trim());
            cmd.Parameters.AddWithValue("Article_Desc", this.tb_Article_Desc.Text);
            cmd.Parameters.AddWithValue("Article_Pic", (ITempList.Count == 0) ? "" : ITempList[0].Param_Pic);
            cmd.Parameters.AddWithValue("Article_PubDate", this.tb_Article_PubDate.Text.ToDateString("yyyy-MM-dd"));
            ...
            //更新本頁Url
            string thisUrl = "{0}Article/Edit/Detail/{1}/{2}/{3}/".FormatThis(
                        Application["WebUrl"]
    ...
    private void Edit_Data(List<TempParam> ITempList)
    {
        using (SqlCommand cmd = new SqlCommand())
        {
            ...
            //[SQL] - 資料更新
            SBSql.AppendLine(" UPDATE Article ");
            SBSql.AppendLine(" SET Article_Title = @Article_Title ");
            SBSql.AppendLine("  , Article_Desc = @Article_Desc, Article_Pic = @Article_Pic, Article_PubDate = @Article_PubDate");
            ...
            cmd.Parameters.AddWithValue("Article_Title", this.tb_Article_Title.Text.Trim());
            cmd.Parameters.AddWithValue("Article_Desc", this.tb_Article_Desc.Text);
            cmd.Parameters.AddWithValue("Article_Pic", (ITempList.Count == 0) ? this.hf_OldFile.Value : ITempList[0].Param_Pic);
            cmd.Parameters.AddWithValue("Article_PubDate", this.tb_Article_PubDate.Text.ToDateString("yyyy-MM-dd"));
    ...
    protected void lbtn_DelFile_Click(object sender, EventArgs e)
    {
        try
        {
            using (SqlCommand cmd = new SqlCommand())
            {
                ...
                //[SQL] - 資料更新
                SBSql.AppendLine(" UPDATE Article SET Article_Pic = NULL WHERE (Article_ID = @DataID) ");
    ...
    protected void btn_BlockSave_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            using (SqlCommand cmd = new SqlCommand())
            {
                ...
                SBSql.AppendLine(" DECLARE @NewID AS INT ");
                SBSql.AppendLine(" SET @NewID = (");
                SBSql.AppendLine("  SELECT ISNULL(MAX(Block_ID) ,0) + 1 FROM Article_Block WHERE (Article_ID = @Article_ID)");
                SBSql.AppendLine(" );");
                SBSql.AppendLine(" SELECT @NewID AS NewID");

                cmd.CommandText = SBSql.ToString();
                cmd.Parameters.AddWithValue("Article_ID", Param_thisID);
                using (DataTable DT = dbConn.LookupDT(cmd, out ErrMsg))
                {
                    NewID = Convert.ToInt32(DT.Rows[0]["NewID"]);
                }
                ...
                SBSql.Append(" INSERT INTO Article_Block( ");
                SBSql.Append("  Article_ID, Block_ID");
                ...
                SBSql.Append(" UPDATE Article SET ");
                SBSql.Append(" Update_Who = @Update_Who, Update_Time = GETDATE() ");
                SBSql.Append(" WHERE (Group_ID = @Group_ID) AND (Article_ID = @Article_ID) ");
        ...
        try
        {
            ...
            using (SqlCommand cmd = new SqlCommand())
            {
                ...
                for (int row = 0; row < lvDataList.Items.Count; row++)
                {
                    ...
                    SBSql.AppendLine(" UPDATE Article_Block SET Sort = @lvParam_Sort_{0}".FormatThis(row));
                    SBSql.AppendLine(" WHERE (Article_ID = @Article_ID) AND (Block_ID = @lvParam_ID_{0}) ".FormatThis(row));
    ...
    public string Page_CurrentUrl
    {
        get
        {
            return "{0}Article/Edit/Detail/{1}/{2}/{3}/".FormatThis(
                Application["WebUrl"]
                , Cryptograph.MD5Encrypt(Param_parentID, Application["DesKey"].ToString())
                , Param_LangCode
    ...
    public string Page_LastUrl
    {
        get
        {
            return "{0}Article/Edit/{1}/".FormatThis(
                Application["WebUrl"]
                , Cryptograph.MD5Encrypt(Param_parentID, Application["DesKey"].ToString()));
        }
        set
    ...
    public string Param_FileFolder
    {
        get
        {
            return this._Param_FileFolder != null
                ...
                : @"{0}Article\{1}\".FormatThis(Application["File_DiskUrl"], Param_parentID);
        }
        set
        {
    ...
    public string Param_WebFolder
    {
        get
        {
            return this._Param_WebFolder != null
                ...
                : @"{0}Article/{1}/".FormatThis(Application["File_WebUrl"], Param_parentID);
        }
        set
        {
