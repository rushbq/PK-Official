...
public partial class Article_Edit : SecurityCheck
{
    ...
    private void LookupData()
    {
        try
        {
            ...
            using (SqlCommand cmd = new SqlCommand())
            {
                ...
                SBSql.AppendLine("            LEFT JOIN Article Base ON Lang.LangCode = Base.LangCode AND Base.Group_ID = GP.Group_ID ");
                SBSql.AppendLine("            WHERE (Lang.Display = 'Y') AND (Base.Article_ID IS NULL) ");
    ...
    private void LookupData_Detail()
    {
        try
        {
            using (SqlCommand cmd = new SqlCommand())
            {
                ...
                SBSql.AppendLine(" SELECT Lang.LangName, Lang.LangCode, Base.Article_ID ");
                SBSql.AppendLine("     , (CASE WHEN Base.Article_ID IS NULL THEN 'N' ELSE 'Y' END) IsSet ");
                SBSql.AppendLine(" FROM PKSYS.dbo.Param_Language Lang ");
                SBSql.AppendLine("     LEFT JOIN Article Base ON Lang.LangCode = Base.LangCode AND Base.Group_ID = @DataID ");
                ...
                using (DataTable DT = dbConn.LookupDT(cmd, out ErrMsg))
                {
                    ...
                    for (int row = 0; row < DT.Rows.Count; row++)
                    {
                        ...
                        string NewsID = DT.Rows[row]["Article_ID"].ToString();
                        ...
                        html.AppendLine("<a href=\"{0}\" class=\"{1}\"><i class=\"{2}\"></i>&nbsp;{3}</a>".FormatThis(
                                "{0}Article/Edit/Detail/{1}/{2}/{3}".FormatThis(
                                        Application["WebUrl"]
    ...
    private void Add_Data()
    {
        using (SqlCommand cmd = new SqlCommand())
        {
            ...
            SBSql.AppendLine("  SELECT ISNULL(MAX(Group_ID) ,0) + 1 FROM Article_Group ");
            SBSql.AppendLine(" );");
            ...
            //[SQL] - 資料新增
            SBSql.AppendLine(" INSERT INTO Article_Group( ");
            ...
            if (GetArea.Count() > 0)
            {
                ...
                foreach (var AreaCode in GetArea)
                {
                    ...
                    SBSql.AppendLine(" INSERT INTO Article_Area( ");
            ...
            {
                //將其他資料皆設為N
                SBSql.AppendLine(" UPDATE Article_Group SET onIndex = 'N' WHERE (Group_ID <> @NewID); ");
            }
            ...
            //更新本頁Url
            string thisUrl = "{0}Article/Edit/{1}/".FormatThis(
                        Application["WebUrl"]
    ...
    private void Edit_Data()
    {
        using (SqlCommand cmd = new SqlCommand())
        {
            ...
            //[SQL] - 資料更新
            SBSql.AppendLine(" UPDATE Article_Group ");
            ...
            {
                //先清除資料
                SBSql.AppendLine(" DELETE FROM Article_Area WHERE (Group_ID = @DataID);");
            ...
            {
                //將其他資料皆設為N
                SBSql.AppendLine(" UPDATE Article_Group SET onIndex = 'N' WHERE (Group_ID <> @DataID); ");
            }
            cmd.Parameters.AddWithValue("onIndex", this.rbl_onIndex.SelectedValue);
    ...
    protected void lbtn_Delete_Click(object sender, EventArgs e)
    {
        try
        {
            using (SqlCommand cmd = new SqlCommand())
            {
                ...
                //[SQL] - 資料更新
                SBSql.AppendLine(" DELETE FROM Article_Block WHERE (Article_ID IN (SELECT Article_ID FROM Article WHERE (Group_ID = @DataID))); ");
                SBSql.AppendLine(" DELETE FROM Article WHERE (Group_ID = @DataID); ");
                SBSql.AppendLine(" DELETE FROM Article_Area WHERE (Group_ID = @DataID); ");
                SBSql.AppendLine(" DELETE FROM Article_Group WHERE (Group_ID = @DataID); ");
                ...
                {
                    //刪除整個Folder
                    string fileUrl = @"{0}Article\{1}\".FormatThis(Application["File_DiskUrl"], Param_thisID);
    ...
    public string Page_SearchUrl
    {
        get
        {
            ...
            {
                Url = "{0}Article/Search/".FormatThis(Application["WebUrl"]);
            }
            else
            {
