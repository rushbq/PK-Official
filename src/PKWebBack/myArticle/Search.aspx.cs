...
public partial class Article_Search : SecurityCheck
{
    ...
    private void LookupDataList(int pageIndex)
    {
        ...
        {
            //[參數宣告] - 設定本頁Url(末端無須加 "/")
            this.ViewState["Page_Url"] = Application["WebUrl"] + "Article/Search";
            ...
            SBSql.AppendLine("    FROM Article_Group Base ");
            ...
            if (Req_Area != null)
            {
                ...
                SBSql.Append(" AND (Base.Group_ID IN (SELECT Area.Group_ID FROM Article_Area Area WHERE (Area.AreaCode IN ({0})))) ".FormatThis(
        ...
        try
        {
            if (e.Item.ItemType == ListViewItemType.DataItem)
            {
                ...
                using (SqlCommand cmd = new SqlCommand())
                {
                    //刪除資料
                    StringBuilder SBSql = new StringBuilder();
                    SBSql.AppendLine(" DELETE FROM Article_Block WHERE (Article_ID IN (SELECT Article_ID FROM Article WHERE (Group_ID = @Param_ID))); ");
                    SBSql.AppendLine(" DELETE FROM Article WHERE (Group_ID = @Param_ID); ");
                    SBSql.AppendLine(" DELETE FROM Article_Area WHERE (Group_ID = @Param_ID); ");
                    SBSql.AppendLine(" DELETE FROM Article_Group WHERE (Group_ID = @Param_ID); ");
                    ...
                    {
                        //刪除整個Folder
                        string fileUrl = @"{0}Article\{1}\".FormatThis(Application["File_DiskUrl"], Get_DataID);
    ...
    protected void btn_Search_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            SBUrl.Append("{0}Article/Search/?srh=1".FormatThis(Application["WebUrl"]));
