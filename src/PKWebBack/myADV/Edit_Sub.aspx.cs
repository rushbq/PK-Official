...
public partial class Adv_Edit_Sub : SecurityCheck
{
    ...
    private void LookupData()
    {
        try
        {
            ...
            using (SqlCommand cmd = new SqlCommand())
            {
                ...
                //[SQL] - 資料查詢
                SBSql.AppendLine(" SELECT Base.Adv_ID,  Base.Adv_Title, Base.Adv_Pic, Base.Adv_Uri");
                SBSql.AppendLine(" FROM ADV Base ");
                SBSql.AppendLine(" WHERE (Base.Adv_ID = @DataID)");
                cmd.CommandText = SBSql.ToString();
                ...
                using (DataTable DT = dbConn.LookupDT(cmd, out ErrMsg))
                {
                    ...
                    {
                        //[填入資料]
                        this.tb_Adv_Title.Text = DT.Rows[0]["Adv_Title"].ToString();
                        this.tb_Adv_Uri.Text = DT.Rows[0]["Adv_Uri"].ToString();

                        //檔案Url - 圖片
                        string myFile = DT.Rows[0]["Adv_Pic"].ToString();
    ...
    protected void btn_Save_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            //[參數檢查] - 必填項目
            if (fn_Extensions.String_資料長度Byte(this.tb_Adv_Title.Text, "1", "200", out ErrMsg) == false)
            {
    ...
    private void Add_Data(List<TempParam> ITempList)
    {
        using (SqlCommand cmd = new SqlCommand())
        {
            ...
            SBSql.AppendLine("  SELECT ISNULL(MAX(Adv_ID) ,0) + 1 FROM Adv ");
            SBSql.AppendLine(" );");
            ...
            //[SQL] - 資料新增
            SBSql.AppendLine(" INSERT INTO Adv( ");
            SBSql.AppendLine("  Group_ID, Adv_ID, LangCode");
            SBSql.AppendLine("  , Adv_Title, Adv_Pic, Adv_Uri");
            ...
            cmd.Parameters.AddWithValue("LangCode", Param_LangCode);
            cmd.Parameters.AddWithValue("Adv_Title", this.tb_Adv_Title.Text.Trim());
            cmd.Parameters.AddWithValue("Adv_Pic", pic1);
            cmd.Parameters.AddWithValue("Adv_Uri", this.tb_Adv_Uri.Text);
            ...
            //更新本頁Url
            string thisUrl = "{0}Adv/Edit/Detail/{1}/{2}/{3}/".FormatThis(
                        Application["WebUrl"]
    ...
    private void Edit_Data(List<TempParam> ITempList)
    {
        using (SqlCommand cmd = new SqlCommand())
        {
            ...
            //[SQL] - 資料更新
            SBSql.AppendLine(" UPDATE Adv ");
            SBSql.AppendLine(" SET Adv_Title = @Adv_Title");
            SBSql.AppendLine("  , Adv_Pic = @Adv_Pic, Adv_Uri = @Adv_Uri");
            SBSql.AppendLine("  , Update_Who = @Update_Who, Update_Time = GETDATE() ");
            SBSql.AppendLine(" WHERE (Adv_ID = @DataID) ");
    ...
    public string Page_CurrentUrl
    {
        get
        {
            return "{0}Adv/Edit/Detail/{1}/{2}/{3}/".FormatThis(
                Application["WebUrl"]
                , Cryptograph.MD5Encrypt(Param_parentID, Application["DesKey"].ToString())
                , Param_LangCode
    ...
    public string Param_FileFolder
    {
        get
        {
            return this._Param_FileFolder != null
                ...
                : @"{0}Adv\{1}\".FormatThis(Application["File_DiskUrl"], Param_parentID);
        }
        set
        {
    ...
    public string Param_WebFolder
    {
        get
        {
            return this._Param_WebFolder != null
                ...
                : @"{0}Adv/{1}/".FormatThis(Application["File_WebUrl"], Param_parentID);
        }
        set
        {
