...
public partial class Adv_Edit : SecurityCheck
{
            ...
            using (SqlCommand cmd = new SqlCommand())
            {
                ...
                using (DataTable DT = dbConn.LookupDT(cmd, out ErrMsg))
                {
                    ...
                    else
                    {
                        ...
                        this.rbl_Adv_Position.SelectedValue = DT.Rows[0]["Adv_Position"].ToString();
                        this.rbl_Adv_Target.SelectedValue = DT.Rows[0]["Adv_Target"].ToString();
    ...
    private void LookupData_Detail()
    {
        try
        {
            using (SqlCommand cmd = new SqlCommand())
            {
                ...
                SBSql.AppendLine(" SELECT Lang.LangName, Lang.LangCode, Base.Adv_ID ");
                SBSql.AppendLine("     , (CASE WHEN Base.Adv_ID IS NULL THEN 'N' ELSE 'Y' END) IsSet ");
                SBSql.AppendLine(" FROM PKSYS.dbo.Param_Language Lang ");
                SBSql.AppendLine("     LEFT JOIN ADV Base ON Lang.LangCode = Base.LangCode AND Base.Group_ID = @DataID ");
                ...
                using (DataTable DT = dbConn.LookupDT(cmd, out ErrMsg))
                {
                    ...
                    for (int row = 0; row < DT.Rows.Count; row++)
                    {
                        ...
                        string KEID = DT.Rows[row]["Adv_ID"].ToString();
                        ...
                        html.AppendLine("<a href=\"{0}\" class=\"{1}\"><i class=\"{2}\"></i>&nbsp;{3}</a>".FormatThis(
                                "{0}Adv/Edit/Detail/{1}/{2}/{3}".FormatThis(
                                        Application["WebUrl"]
                                        , Cryptograph.MD5Encrypt(Param_thisID, Application["DesKey"].ToString())
                                        , DT.Rows[row]["LangCode"].ToString()
                                        , string.IsNullOrEmpty(KEID) ? "" : Cryptograph.MD5Encrypt(KEID, Application["DesKey"].ToString())
                                    )
                                , urlCss
                                , iconCss
                    ...
                    //輸出Html
                    this.lt_LangSet.Text = html.ToString();
                }
            }
        }
        catch (Exception)
    ...
    private void Add_Data()
    {
        using (SqlCommand cmd = new SqlCommand())
        {
            ...
            SBSql.AppendLine("  SELECT ISNULL(MAX(Group_ID) ,0) + 1 FROM Adv_Group ");
            SBSql.AppendLine(" );");
            ...
            //[SQL] - 資料新增
            SBSql.AppendLine(" INSERT INTO Adv_Group( ");
            SBSql.AppendLine("  Group_ID, Adv_Position, Adv_Target, Group_Name, StartTime, EndTime, Display, Sort");
            SBSql.AppendLine("  , Create_Who, Create_Time");
            SBSql.AppendLine(" ) VALUES ( ");
            SBSql.AppendLine("  @NewID, @Adv_Position, @Adv_Target, @Group_Name, @StartTime, @EndTime, @Display, @Sort");
            ...
            if (GetArea.Count() > 0)
            {
                ...
                foreach (var AreaCode in GetArea)
                {
                    ...
                    SBSql.AppendLine(" INSERT INTO ADV_Area( ");
            ...
            cmd.Parameters.AddWithValue("Adv_Position", this.rbl_Adv_Position.SelectedValue);
            cmd.Parameters.AddWithValue("Adv_Target", this.rbl_Adv_Target.SelectedValue);
            ...
            //更新本頁Url
            string thisUrl = "{0}Adv/Edit/{1}/".FormatThis(
                        Application["WebUrl"]
    ...
    private void Edit_Data()
    {
        using (SqlCommand cmd = new SqlCommand())
        {
            ...
            //[SQL] - 資料更新
            SBSql.AppendLine(" UPDATE Adv_Group ");
            SBSql.AppendLine(" SET Adv_Position = @Adv_Position, Adv_Target = @Adv_Target, Group_Name = @Group_Name ");
            ...
            {
                //先清除資料
                SBSql.AppendLine(" DELETE FROM ADV_Area WHERE (Group_ID = @DataID);");
    ...
    protected void lbtn_Delete_Click(object sender, EventArgs e)
    {
        try
        {
            using (SqlCommand cmd = new SqlCommand())
            {
                ...
                //[SQL] - 資料更新
                SBSql.AppendLine(" DELETE FROM ADV WHERE (Group_ID = @DataID); ");
                SBSql.AppendLine(" DELETE FROM ADV_Area WHERE (Group_ID = @DataID); ");
                SBSql.AppendLine(" DELETE FROM ADV_Group WHERE (Group_ID = @DataID); ");
                ...
                {
                    //刪除整個Folder
                    string fileUrl = @"{0}Adv\{1}\".FormatThis(Application["File_DiskUrl"], Param_thisID);
    ...
    public string Page_CurrentUrl
    {
        get
        {
            return "{0}Adv/Edit/{1}/".FormatThis(
                Application["WebUrl"]
    ...
    public string Page_SearchUrl
    {
        get
        {
            ...
            {
                Url = "{0}Adv/Search/".FormatThis(Application["WebUrl"]);
            }
            else
            {
