...
public partial class Adv_Search : SecurityCheck
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            if (!IsPostBack)
            {
                ...
                //[取得/檢查參數] - 廣告位置
                if (!string.IsNullOrEmpty(Req_Pos))
                {
                    this.ddl_AdvPos.SelectedValue = Req_Pos;
                }
    ...
    private void LookupDataList(int pageIndex)
    {
        ...
        {
            //[參數宣告] - 設定本頁Url(末端無須加 "/")
            this.ViewState["Page_Url"] = Application["WebUrl"] + "Adv/Search";
            ...
            SBSql.AppendLine("      Base.Group_ID, Base.Adv_Position, Base.Group_Name, Base.StartTime, Base.EndTime, Base.Display ");
            SBSql.AppendLine("      , ROW_NUMBER() OVER (ORDER BY Base.Display DESC, Base.Group_ID DESC) AS RowRank ");
            SBSql.AppendLine("    FROM ADV_Group Base ");
            ...
            if (!string.IsNullOrEmpty(Req_Pos))
            {
                SBSql.Append(" AND (Base.Adv_Position = @Position) ");
                cmd.Parameters.AddWithValue("Position", Req_Pos);

                Params.Add("Pos=" + Server.UrlEncode(Req_Pos));
            }
            ...
            if (Req_Area != null)
            {
                ...
                SBSql.Append(" AND (Base.Group_ID IN (SELECT Area.Group_ID FROM ADV_Area Area WHERE (Area.AreaCode IN ({0})))) ".FormatThis(
            ...
            {
                SBSql.Append(" AND (Base.Adv_Position = @Position) ");
                cmdTotalCnt.Parameters.AddWithValue("Position", Req_Pos);
            }
        ...
        try
        {
            if (e.Item.ItemType == ListViewItemType.DataItem)
            {
                ...
                using (SqlCommand cmd = new SqlCommand())
                {
                    //刪除資料
                    StringBuilder SBSql = new StringBuilder();
                    SBSql.AppendLine(" DELETE FROM ADV WHERE (Group_ID = @Param_ID); ");
                    SBSql.AppendLine(" DELETE FROM ADV_Area WHERE (Group_ID = @Param_ID); ");
                    SBSql.AppendLine(" DELETE FROM ADV_Group WHERE (Group_ID = @Param_ID); ");
                    ...
                    {
                        //刪除整個Folder
                        string fileUrl = @"{0}ADV\{1}\".FormatThis(Application["File_DiskUrl"], Get_DataID);
    ...
    /// <returns></returns>
    protected string Get_AdvPos(string pos) {
        switch (pos)
        {
            case "1":
                return "橫幅廣告";
    ...
    protected void btn_Search_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            SBUrl.Append("{0}Adv/Search/?srh=1".FormatThis(Application["WebUrl"]));
            ...
            //[查詢條件] - Position
            if (this.ddl_AdvPos.SelectedIndex > 0)
            {
                SBUrl.Append("&Pos=" + Server.UrlEncode(this.ddl_AdvPos.SelectedValue));
            }
    ...
    /// 取得傳遞參數 - Pos
    /// </summary>
    private string _Req_Pos;
    public string Req_Pos
    {
        get
        {
            String GetReq = Request.QueryString["Pos"];
            return (fn_Extensions.String_資料長度Byte(GetReq, "1", "1", out ErrMsg)) ? fn_stringFormat.Set_FilterHtml(GetReq).Trim() : "";
        }
        set
        {
            this._Req_Pos = value;
