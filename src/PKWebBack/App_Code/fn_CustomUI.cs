...
namespace ExtensionUI
{
    ...
    public class fn_CustomUI
    {
        ...
        /// DB = PKSYS
        /// </remarks>
        public static bool Get_Area(CheckBoxList setMenu, string[] inputValues, out string ErrMsg)
        {
            //清除參數
            ErrMsg = "";
            setMenu.Items.Clear();
        ...
        /// TW/CN/EN
        /// </remarks>
        public static bool Get_Area(RadioButtonList setMenu, string inputValue, out string ErrMsg)
        {
            //清除參數
            ErrMsg = "";
            setMenu.Items.Clear();
        ...
        /// DB = PKWeb
        /// </remarks>
        public static bool Get_Region(DropDownList setMenu, string inputValue, bool showRoot, out string ErrMsg)
        {
            //清除參數
            ErrMsg = "";
            setMenu.Items.Clear();
            ...
            try
            {
                ...
                using (SqlCommand cmd = new SqlCommand())
                {
                    ...
                    using (DataTable DT = dbConn.LookupDT(cmd, out ErrMsg))
                    {
                        ...
                        if (showRoot)
                        {
                            setMenu.Items.Insert(0, new ListItem("-- 選擇洲別 --", ""));
                        }
                    }
                }
        ...
        /// DB = PKWeb
        /// </remarks>
        public static bool Get_Country(DropDownList setMenu, string inputValue, string code, bool showRoot, out string ErrMsg)
        {
            //清除參數
            ErrMsg = "";
            setMenu.Items.Clear();
            ...
            try
            {
                ...
                using (SqlCommand cmd = new SqlCommand())
                {
                    ...
                    // ↓↓ SQL查詢組成 ↓↓
                    SBSql.AppendLine(" SELECT Sub.Country_Code AS ID, Sub.Country_Name AS Label");
                    ...
                    SBSql.AppendLine(" WHERE (LOWER(Sub.LangCode) = LOWER('zh-tw')) AND (Base.AreaCode = @areaCode)");
                    SBSql.AppendLine(" ORDER BY Sub.Country_Code");

                    cmd.CommandText = SBSql.ToString();
                    cmd.Parameters.AddWithValue("areaCode", code.ToUpper());
                    // ↑↑ SQL查詢組成 ↑↑
                    ...
                    using (DataTable DT = dbConn.LookupDT(cmd, out ErrMsg))
                    {
                        if (DT.Rows.Count == 0)
                        {
                            setMenu.Items.Insert(0, new ListItem("查無國家...", ""));
                        }
                        else
                        {
                            //新增選單項目
                            ...
                            if (showRoot)
                            {
                                setMenu.Items.Insert(0, new ListItem("-- 選擇國家 --", ""));
                            }
                        }
        ...
        public static bool Get_City(DropDownList setMenu, string inputValue, string code, bool showRoot, out string ErrMsg)
        {
            ...
            try
            {
                ...
                using (SqlCommand cmd = new SqlCommand())
                {
                    ...
                    // ↓↓ SQL查詢組成 ↓↓
                    SBSql.AppendLine(" SELECT Region_Code AS ID, Region_Name_zh_TW AS Label ");
                    ...
                    using (DataTable DT = dbConn.LookupDT(cmd, out ErrMsg))
                    {
                        if (DT.Rows.Count == 0)
                        {
                            setMenu.Items.Insert(0, new ListItem("查無地區選項,非必填", ""));
                        }
                        else
                        {
                            //新增選單項目
                            ...
                            if (showRoot)
                            {
                                setMenu.Items.Insert(0, new ListItem("-- 選擇地區/城市 --", ""));
                            }
