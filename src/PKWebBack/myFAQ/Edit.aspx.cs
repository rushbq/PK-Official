...
public partial class FAQ_Edit : SecurityCheck
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            if (!IsPostBack)
            {
                ...
                //[取得/檢查參數] - 類別
                if (fn_CustomUI.Get_FAQClass(this.ddl_FAQClass, "", false, out ErrMsg) == false)
                {
    ...
    private void LookupData()
    {
        try
        {
            ...
            using (SqlCommand cmd = new SqlCommand())
            {
                ...
                SBSql.AppendLine("      INNER JOIN FAQ_Area Area ON GP.Group_ID = Area.Group_ID ");
                ...
                using (DataTable DT = dbConn.LookupDT(cmd, out ErrMsg))
                {
                    ...
                    else
                    {
                        ...
                        this.ddl_FAQClass.SelectedValue = DT.Rows[0]["Class_ID"].ToString();
                ...
                using (DataTable DT = dbConn.LookupDT(cmd, out ErrMsg))
                {
                    ...
                    for (int row = 0; row < DT.Rows.Count; row++)
                    {
                        ...
                        html.AppendLine("<a href=\"{0}\" class=\"{1}\"><i class=\"{2}\"></i>&nbsp;{3}</a>".FormatThis(
                                "{0}FAQ/Edit/Detail/{1}/{2}/{3}".FormatThis(
                                        Application["WebUrl"]
                                        , Cryptograph.MD5Encrypt(Param_thisID, Application["DesKey"].ToString())
                                        , DT.Rows[row]["LangCode"].ToString()
                                        , string.IsNullOrEmpty(FAQID) ? "" : Cryptograph.MD5Encrypt(FAQID, Application["DesKey"].ToString())
                                    )
                                , urlCss
                                , iconCss
                    ...
                    //輸出Html
                    this.lt_FAQSet.Text = html.ToString();
                }
            }
        }
        catch (Exception)
    ...
    protected void btn_Save_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            //[參數檢查] - 必填項目
            if (fn_Extensions.String_資料長度Byte(this.tb_Group_Name.Text, "1", "200", out ErrMsg) == false)
            {
                SBAlert.Append("「名稱」請輸入1 ~ 100個字\\n");
            }
    ...
    private void Add_Data()
    {
        using (SqlCommand cmd = new SqlCommand())
        {
            ...
            SBSql.AppendLine("  Group_ID, Group_Name, Class_ID, Target, Display, Sort");
            SBSql.AppendLine("  , Create_Who, Create_Time");
            SBSql.AppendLine(" ) VALUES ( ");
            SBSql.AppendLine("  @NewID, @Group_Name, @Class_ID, @Target, @Display, @Sort");
            ...
            if (GetArea.Count() > 0)
            {
                ...
                foreach (var AreaCode in GetArea)
                {
                    ...
                    SBSql.AppendLine(" INSERT INTO FAQ_Area( ");
            ...
            cmd.Parameters.AddWithValue("Class_ID", this.ddl_FAQClass.SelectedValue);
            ...
            //更新本頁Url
            string thisUrl = "{0}FAQ/Edit/{1}/".FormatThis(
                        Application["WebUrl"]
    ...
    private void Edit_Data()
    {
        using (SqlCommand cmd = new SqlCommand())
        {
            ...
            SBSql.AppendLine("  , Class_ID = @Class_ID, Target = @Target, Display = @Display, Sort = @Sort");
            ...
            {
                //先清除資料
                SBSql.AppendLine(" DELETE FROM FAQ_Area WHERE (Group_ID = @DataID);");
    ...
    protected void lbtn_Delete_Click(object sender, EventArgs e)
    {
        try
        {
            using (SqlCommand cmd = new SqlCommand())
            {
                ...
                //[SQL] - 資料刪除
                SBSql.AppendLine(" DELETE FROM FAQ_FeedBack WHERE (FAQ_ID IN (SELECT FAQ_ID FROM FAQ WHERE (Group_ID = @DataID)))");
    ...
    public string Page_SearchUrl
    {
        get
        {
            ...
            {
                Url = "{0}FAQ/Search/".FormatThis(Application["WebUrl"]);
            }
            else
            {
